...
allwksesterr = std(allwks(:,1:end-1)-allwksest(:,1:end-1),[],2);
allwkspuc = mean(allwks(:,6:10),2);
allwkspucerr = std(allwks(:,6:10)-allwkspuc,[],2);

%% error cuadratico medio durante ultimos 5 anyos
errpuca = norm(allwks(:,6:10)-allwkspuc,'fro');
erresti = norm(allwks(:,6:10)-allwksest(:,6:10),'fro');
fprintf('El error en ultimos 5 años de PUCA es %.3g%% mas grande que error de ajuste\n',(errpuca./erresti-1)*100)
%% error cuadratico medio durante ultimos 10 anyos
errpuca = norm(allwks(:,1:10)-allwkspuc,'fro');
erresti = norm(allwks(:,1:10)-allwksest(:,1:10),'fro');
fprintf('El error en ultimos 10 años de PUCA es %.3g%% mas grande que error de ajuste\n',(errpuca./erresti-1)*100)
%% error cuadratico medio durante 10 primeras semanas 2020
errpuca = norm(allwks(1:10,end)-allwkspuc(1:10),'fro');
erresti = norm(allwks(1:10,end)-allwksest(1:10,end),'fro');
fprintf('El error en primeras 10 semanas 2020 de PUCA es %.3g%% mas grande que error de ajuste\n',(errpuca./erresti-1)*100)
%%
...
subplot 211
xax = (1:size(allwks,1))';
ydat = allwksest(:,end) + [-1 1].*allwksesterr; %/sqrt(size(allwks,2)-1);
xax = xax(:);
ydat = ydat(:);
cli = colormap(lines);
...
subplot 212
xax = (1:size(allwks,1))';
ydat = allwkspuc + [-1 1].*allwkspucerr;%/sqrt(size(allwks,2)-1);
xax = xax(:);
ydat = ydat(:);
cli = colormap(lines);
plot(xax,[allwkspuc],'color',cli(1,:),'linewidth',2)
hold on
plot(xax,[allwks(:,end)],'color',cli(2,:),'linewidth',2)
patch([xax; xax(end:-1:1)],[ydat(1:end/2); ydat(end:-1:end/2+1)],cli(1,:),'edgecolor','none','facealpha',.2)
ylim(ylim.*[0 1])
...
xlabel 'Semana'
ylabel 'Defunciones'
legend({'puca',['Defunciones 2020 (hasta ' fecha ')']},'location','se')
title({'Defunciones por cualquier causa registradas en Chile','fuente: estadísticas Registro Civil'})
